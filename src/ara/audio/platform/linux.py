"""Linux/ALSA audio backend using PyAudio.

Provides AudioCapture and AudioPlayback implementations for Linux
...
class LinuxAudioCapture:
    """Linux audio capture using PyAudio/ALSA.

    Implements the AudioCapture protocol for Linux systems including
    Raspberry Pi. Optimized for low-latency capture with ALSA.
    """
    ...
    def __init__(
        ...
        Args:
            device_name: ALSA device name (e.g., "default", "hw:0,0",
                        "seeed-4mic-voicecard" for ReSpeaker)
    ...
    def _get_device_index(self) -> int | None:
        """Get ALSA device index for configured device name."""
        ...
        for i in range(self._pa.get_device_count()):
            try:
                ...
                search_name_lower = self._device_name.lower()

                # Match by partial name or ALSA device string
                if (
                    search_name_lower in device_name_lower
                    or device_name_lower.startswith(search_name_lower)
    ...
    def start(self) -> None:
        ...
        # Linux/ALSA specific settings for better latency
    ...
    def read(self, frames: int) -> AudioChunk:
        ...
        # Use exception_on_overflow=False to avoid errors on slow systems
    ...
    def __enter__(self) -> "LinuxAudioCapture":
...
class LinuxAudioPlayback:
    """Linux audio playback using PyAudio/ALSA.

    Implements the AudioPlayback protocol for Linux systems including
    Raspberry Pi. Supports ReSpeaker and other USB audio devices.
    """
    ...
    def __init__(
        ...
        """Initialize Linux audio playback.

        Args:
            device_name: ALSA device name or "default"
...
__all__ = ["LinuxAudioCapture", "LinuxAudioPlayback"]
